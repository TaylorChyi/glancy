...
/**
 * 测试全局异常处理在 SSE 请求下的返回。 流程： 1. 发送 Accept 为 text/event-stream 的请求。 2. 控制器抛出
 * ResourceNotFoundException。 3. 验证响应为 SSE 格式错误事件。
 */
...
class GlobalExceptionHandlerSseTest {
  ...
  void returnsSseErrorWhenAcceptEventStream() throws Exception {
    mvc.perform(get("/dummy/boom").accept(MediaType.TEXT_EVENT_STREAM))
