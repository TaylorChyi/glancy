...
class SearchRecordRepositoryTest {
  ...
  void searchRecordQueries() {
    ...
    SearchRecord r1 =
        TestEntityFactory.searchRecord(
            user, "term1", Language.ENGLISH, LocalDateTime.now().minusDays(1));
    SearchRecord r2 =
        TestEntityFactory.searchRecord(user, "term2", Language.ENGLISH, LocalDateTime.now());
    ...
    long count =
        searchRecordRepository.countByUserIdAndCreatedAtBetween(
            user.getId(), LocalDateTime.now().minusDays(2), LocalDateTime.now());
    assertEquals(2, count);

    assertTrue(
        searchRecordRepository.existsByUserIdAndTermAndLanguage(
            user.getId(), "term1", Language.ENGLISH));

    SearchRecord r3 =
        TestEntityFactory.searchRecord(
            user, "term1", Language.ENGLISH, LocalDateTime.now().plusMinutes(1));
