...
class PromptTemplateRendererTest {
    ...
    @Test
    /**
     * 测试目标：渲染器应正确替换模板中的占位符。
     * 前置条件：提供 USER_GOAL 模板与含 goal 的上下文。
     * 步骤：
     *  1) 使用不同上下文渲染模板两次。
     * 断言：
     *  - 两次渲染结果分别包含对应的目标内容。
    ...
    @Test
    /**
     * 测试目标：当上下文缺失占位符时应退化为空字符串。
     * 前置条件：提供 USER_GOAL 模板与空上下文。
     * 步骤：
     *  1) 渲染模板并检查输出。
     * 断言：
     *  - 输出中的变量位置为空。
    ...
    @Test
    /**
     * 测试目标：验证字符串模板重载可以处理行内模板。
     * 前置条件：提供含单一占位符的字符串模板与上下文。
     * 步骤：
     *  1) 调用字符串重载渲染模板。
     * 断言：
     *  - 渲染结果中占位符被替换。
