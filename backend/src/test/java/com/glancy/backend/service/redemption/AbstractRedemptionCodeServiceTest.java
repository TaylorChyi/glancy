...
abstract class AbstractRedemptionCodeServiceTest {
  ...
  @Autowired protected RedemptionCodeService redemptionCodeService;

  @Autowired protected RedemptionCodeRepository redemptionCodeRepository;

  @Autowired protected RedemptionRecordRepository redemptionRecordRepository;
  ...
  protected RedemptionCodeCreateRequest membershipCode(MembershipCodeParams params) {
    RedemptionEffectConfig effect =
        new RedemptionEffectConfig(
  ...
  protected void assertDiscountBenefit(
    ...
    Assertions.assertThat(response.discount().percentage())
        .isEqualByComparingTo(expectedPercentage);
    Assertions.assertThat(benefits).hasSize(1);
    Assertions.assertThat(benefits.get(0).getDiscountPercentage())
        .isEqualByComparingTo(expectedPercentage);
