...
class KeyboardShortcutServiceTest {
    ...
    /**
     * 测试目标：首次读取快捷键时应返回所有默认配置。 前置条件：数据库中不存在用户快捷键覆盖记录。 步骤： 1) 创建用户； 2) 调用 getShortcuts。 断言： -
     * 返回列表长度与枚举数量一致； - 每项 keys 与 defaultKeys 相同； - 默认组合包含主按键。 边界/异常： - 若用户不存在应抛出
     * ResourceNotFoundException（由控制层覆盖）。
     */
    @Test
    void Given_noOverrides_When_getShortcuts_Then_returnDefaults() {
        ...
        Assertions.assertEquals(
                ShortcutAction.values().length, response.shortcuts().size(), "shortcut size");
        Assertions.assertTrue(
                response.shortcuts().stream()
    ...
    /**
     * 测试目标：更新快捷键后应持久化并在再次查询时返回新绑定。 前置条件：用户存在且使用默认快捷键。 步骤： 1) 创建用户并调用 updateShortcut 绑定新组合； 2) 再次调用
     * getShortcuts； 3) 查找对应动作的 keys。 断言： - 更新后的 keys 与提交的组合一致； - 其他动作仍保持默认值。 边界/异常： - 如提交重复主键应触发
     * InvalidRequestException（由下一用例覆盖）。
     */
    @Test
    void Given_validUpdate_When_updateShortcut_Then_overridePersisted() {
        ...
        Assertions.assertTrue(
                response.shortcuts().stream()
                        .anyMatch(view -> view.action().equals(ShortcutAction.FOCUS_SEARCH.name())
    ...
    /**
     * 测试目标：当新组合与其它动作重复时应拒绝更新。 前置条件：用户已存在，且某动作使用默认组合。 步骤： 1) 创建用户并将 SWITCH_LANGUAGE 绑定为自定义组合； 2) 再次尝试将
     * FOCUS_SEARCH 更新为相同组合； 断言： - updateShortcut 抛出 InvalidRequestException； - 数据库中原绑定不受影响。 边界/异常： -
     * 若换成不同组合则应通过（由上一用例验证）。
     */
    @Test
    void Given_conflictingBinding_When_updateShortcut_Then_throwInvalidRequest() {
        ...
        KeyboardShortcutResponse response = keyboardShortcutService.getShortcuts(user.getId());
        Assertions.assertTrue(response.shortcuts().stream()
                .anyMatch(view -> view.action().equals(ShortcutAction.SWITCH_LANGUAGE.name())
                        && view.keys().equals(List.of("CONTROL", "SHIFT", "Q"))));
    }

    /**
     * 测试目标：当客户端尝试使用与 MOD 默认键等价的组合时应识别冲突。 前置条件：用户存在且尚未自定义 OPEN_SHORTCUTS，默认键位为 MOD+SHIFT+K。 步骤： 1) 以
     * CONTROL+SHIFT+F 更新 OPEN_SHORTCUTS； 断言： - 服务抛出 InvalidRequestException； - OPEN_SHORTCUTS
     * 仍维持默认绑定。 边界/异常： - 若先修改冲突方（FOCUS_SEARCH）应允许后续操作，此处不覆盖。
     */
    @Test
    void Given_modAliasConflict_When_updateShortcut_Then_throwInvalidRequest() {
        ...
        Assertions.assertTrue(
                response.shortcuts().stream()
                        .anyMatch(view -> view.action().equals(ShortcutAction.OPEN_SHORTCUTS.name())
