...
class OssAvatarStorageTest {
  ...
  void aclDeniedDoesNotThrow() throws Exception {
    ...
    Mockito.when(client.putObject(eq("bucket"), anyString(), any(java.io.InputStream.class)))
        .thenReturn(null);
    OSSException ex = new OSSException("AccessDenied");
    Mockito.doThrow(ex)
        .when(client)
        .setObjectAcl(eq("bucket"), anyString(), eq(CannedAccessControlList.PublicRead));
    Mockito.when(client.generatePresignedUrl(any(GeneratePresignedUrlRequest.class)))
        .thenReturn(new java.net.URL("https://example.com"));

    MockMultipartFile file =
        new MockMultipartFile("file", "avatar.jpg", "image/jpeg", "data".getBytes());
