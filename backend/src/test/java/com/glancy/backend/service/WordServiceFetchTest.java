...
class WordServiceFetchTest extends BaseWordServiceTest {
    ...
    void testFetchAndCacheWord() {
        WordResponse result = wordService.findWordForUser(
                userId, options("hello", Language.ENGLISH, DictionaryFlavor.BILINGUAL, null, false, true));

        Assertions.assertNotNull(result.getId());
        Assertions.assertTrue(
                wordRepository.findById(Long.parseLong(result.getId())).isPresent());
    }
    ...
    void testUseCachedWord() {
        ...
        WordResponse result = wordService.findWordForUser(
                userId, options("CACHED", Language.ENGLISH, DictionaryFlavor.BILINGUAL, null, false, true));
    ...
    void testCacheWordWhenLanguageMissing() {
        WordResponse result = wordService.findWordForUser(
                userId, options("bye", Language.ENGLISH, DictionaryFlavor.BILINGUAL, null, false, true));

        Assertions.assertEquals(Language.ENGLISH, result.getLanguage());
        Assertions.assertTrue(wordRepository
                .findActiveByNormalizedTerm("bye", Language.ENGLISH, DictionaryFlavor.BILINGUAL)
                .isPresent());
    }
    ...
    void testFindWordSkipsHistoryWhenDisabled() {
        wordService.findWordForUser(
                userId, options("skip", Language.ENGLISH, DictionaryFlavor.BILINGUAL, null, false, false));
