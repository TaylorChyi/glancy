...
class TemplateDrivenVerificationEmailContentResolverTest {
  ...
  void setUp() {
    ...
    LinkedHashMap<String, EmailVerificationLocalizationProperties.Message> messages =
        new LinkedHashMap<>();
  ...
  /**
   * 测试目标：当解析出的语言为日语时返回日语验证码文案。 前置条件：配置中包含 zh-CN、ja-JP 模板，默认语言 zh-CN。 步骤： 1) 构造固定返回日语 Locale 的解析器；
   * 2) 调用渲染器生成内容。 断言： - 纯文本为 "認証コード：123456"； - HTML 包含同样的内容。 边界/异常： - 无特别边界。
   */
  @Test
  void Given_JapaneseLocale_When_Resolve_Then_ReturnJapaneseMessage() {
    VerificationEmailContentResolver resolver =
        new TemplateDrivenVerificationEmailContentResolver(
            properties, ignored -> Locale.forLanguageTag("ja-JP"));
  ...
  /**
   * 测试目标：当解析出的语言缺失模板时回退到默认语言。 前置条件：默认语言为 zh-CN，且缺少 fr-FR 模板。 步骤： 1) 构造固定返回法语 Locale 的解析器； 2)
   * 调用渲染器生成内容。 断言： - 纯文本回退为中文模板； - HTML 同样为中文内容。 边界/异常： - 验证回退逻辑。
   */
  @Test
  void Given_MissingLocale_When_Resolve_Then_FallBackToDefault() {
    VerificationEmailContentResolver resolver =
        new TemplateDrivenVerificationEmailContentResolver(
            properties, ignored -> Locale.forLanguageTag("fr-FR"));
  ...
  private Message createMessage(String body) {
    EmailVerificationLocalizationProperties.Message message =
        new EmailVerificationLocalizationProperties.Message();
