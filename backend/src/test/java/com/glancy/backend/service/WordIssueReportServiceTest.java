...
class WordIssueReportServiceTest {
    ...
    @Autowired
    private WordIssueReportService service;

    @Autowired
    private WordIssueReportRepository repository;
    ...
    /**
     * 测试目标：验证 registerReport 会持久化举报并返回包含主键与裁剪后的字段。
     * 前置条件：构造必填字段齐全的请求体，描述带有首尾空格。
     * 步骤：
     *  1) 调用 registerReport;
     *  2) 读取数据库中唯一一条记录。
     * 断言：
