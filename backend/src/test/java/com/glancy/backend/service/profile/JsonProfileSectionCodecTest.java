...
class JsonProfileSectionCodecTest {
  ...
  /**
   * 测试目标：验证正常场景下 JSON 序列化与反序列化保持信息完整。 前置条件：构造包含一个大项与两个小项的列表。 步骤： 1) 调用 serialize 获取 JSON； 2) 调用
   * deserialize 还原列表。 断言： - 反序列化后列表长度与标题保持一致； - 小项内容与原始值一致。 边界/异常：若未来引入版本字段，此处需补充兼容性用例。
   */
  @Test
  void serializeAndDeserializeRoundTrip() {
    List<ProfileCustomSectionDto> sections =
        List.of(
  ...
  /**
   * 测试目标：确保空列表序列化后返回 null，反序列化 null 或空串时返回空列表。 前置条件：提供空集合与空字符串。 步骤： 1) 调用 serialize(List.of()); 2)
   * 分别调用 deserialize(null) 与 deserialize(""). 断言：均返回空列表。 边界/异常：若后续需要区分「未保存」与「已保存为空」，可在 codec
   * 中引入标记串。
   */
  @Test
  void handleEmptyPayload() {
    ...
    Assertions.assertTrue(
        codec.deserialize(null).isEmpty(), "null payload should become empty list");
    Assertions.assertTrue(
        codec.deserialize(" ").isEmpty(), "blank payload should become empty list");
  }
}
