...
class UserControllerRegistrationTest extends BaseUserControllerWebMvcTest {
    ...
    void register() throws Exception {
        UserResponse resp =
                new UserResponse(1L, "testuser", "test@example.com", null, "555", false, MembershipType.NONE, null);
    ...
    void getUser() throws Exception {
        UserDetailResponse detailResponse = UserDetailResponse.of(
                ...
                new UserDetailResponse.Audit(
                        false, LocalDateTime.of(2024, 1, 1, 8, 0), LocalDateTime.of(2024, 1, 2, 9, 0), null));
    ...
    void bindThirdParty() throws Exception {
        ...
        when(userService.bindThirdPartyAccount(eq(1L), any(ThirdPartyAccountRequest.class)))
                .thenReturn(resp);
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/api/users/1/third-party-accounts")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
                .andExpect(MockMvcResultMatchers.status().isCreated())
                .andExpect(MockMvcResultMatchers.jsonPath("$.id").value(1L));
    ...
    void updateContact() throws Exception {
        ...
        when(userService.updateContact(eq(1L), eq("changed@example.com"), eq("7654321")))
                .thenReturn(resp);

        UserContactRequest req = new UserContactRequest("changed@example.com", "7654321");

        mockMvc.perform(MockMvcRequestBuilders.put("/api/users/1/contact")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
                .andExpect(MockMvcResultMatchers.status().isOk())
    ...
    void requestEmailChangeCode() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/api/users/1/email/change-code")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
    ...
    void changeEmail() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.put("/api/users/1/email")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
                .andExpect(MockMvcResultMatchers.status().isOk())
    ...
    void unbindEmail() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.delete("/api/users/1/email"))
                .andExpect(MockMvcResultMatchers.status().isOk())
