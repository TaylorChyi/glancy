...
/**
 * Tests for {@link TtsController}. The focus is on verifying the HTTP contract and parameter
 * handling rather than synthesis logic.
 */
...
class TtsControllerTest {
    ...
    /** Verify that a successful word synthesis returns the payload from the service layer. */
    @Test
    void synthesizeWordReturnsAudio() throws Exception {
        ...
        when(ttsService.synthesizeWord(eq(1L), anyString(), any(TtsRequest.class)))
                .thenReturn(Optional.of(resp));
        when(userService.authenticateToken("tkn")).thenReturn(1L);

        mockMvc.perform(post("/api/tts/word")
                ...
                .andExpect(
                        jsonPath("$.data").value(java.util.Base64.getEncoder().encodeToString(data)));
    }

    /**
     * When the service indicates a cache miss, the controller should respond with 204 to signal the
     * client to retry without shortcut.
     */
    @Test
    ...
    /** Ensure the voices endpoint returns voice data as provided by the service. */
    @Test
    void listVoicesReturnsOptions() throws Exception {
        ...
        mockMvc.perform(get("/api/tts/voices").header("X-USER-TOKEN", "tkn").param("lang", "en"))
                .andExpect(status().isOk())
    ...
    void streamSentenceReturnsPayload() throws Exception {
        ...
        when(ttsService.synthesizeSentence(eq(1L), anyString(), any(TtsRequest.class)))
                .thenReturn(Optional.of(resp));
        when(userService.authenticateToken("tkn")).thenReturn(1L);

        mockMvc.perform(get("/api/tts/sentence")
    ...
    void streamWordReturnsPayload() throws Exception {
        ...
        mockMvc.perform(get("/api/tts/word")
                        .param("text", "hello")
                        .param("lang", "en")
                        .header("X-USER-TOKEN", "tkn"))
                .andExpect(status().isOk())
