...
class WordControllerTest {
    ...
    void testGetWord() throws Exception {
        performWordLookup(defaultOptions("hello"), response("1", "hello"), builder -> builder.withHeaderToken("tkn")
                        .withTerm("hello")
    ...
    void testGetWordWithModel() throws Exception {
        performWordLookup(
                        WordSearchOptions.of(
                                "hello", Language.ENGLISH, DictionaryFlavor.BILINGUAL, "doubao", false, true),
                        response("1", "hello"),
                        builder -> builder.withHeaderToken("tkn")
                                .withTerm("hello")
                                .withLanguage(Language.ENGLISH)
                                .withModel("doubao"))
                .andDo(print())
    ...
    void whenCaptureHistoryDisabled_thenHistoryFlagFalse() throws Exception {
        WordSearchOptions options = captureHistoryOptions("hello", false);
        performCaptureHistoryDisabledLookup(options);
        Mockito.verify(wordService).findWordForUser(ArgumentMatchers.eq(1L), ArgumentMatchers.eq(options));
    }

    private ResultActions performQueryTokenLookup() throws Exception {
        return performWordLookup(defaultOptions("hi"), response("1", "hi"), builder -> builder.withQueryToken("tkn")
                .withTerm("hi")
                .withLanguage(Language.ENGLISH));
    }

    private ResultActions performCaptureHistoryDisabledLookup(WordSearchOptions options) throws Exception {
        return performWordLookup(options, response("1", "hello"), builder -> builder.withHeaderToken("tkn")
                .withTerm("hello")
                .withLanguage(Language.ENGLISH)
    ...
    private ResultActions performWordLookup(
            WordSearchOptions expectedOptions, WordResponse resp, Consumer<WordRequestBuilder> customizer)
            throws Exception {
        Mockito.when(wordService.findWordForUser(ArgumentMatchers.eq(1L), ArgumentMatchers.eq(expectedOptions)))
                .thenReturn(resp);
    ...
    private WordSearchOptions captureHistoryOptions(String term, boolean captureHistory) {
        return WordSearchOptions.of(term, Language.ENGLISH, DictionaryFlavor.BILINGUAL, null, false, captureHistory);
    }
