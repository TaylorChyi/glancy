...
class UserProfileControllerTest {
    ...
    void saveProfile() throws Exception {
        when(userProfileService.saveProfile(eq(2L), any(UserProfileRequest.class)))
                .thenReturn(profileResponse());

        mockMvc.perform(MockMvcRequestBuilders.post("/api/profiles/user")
    ...
    /**
     * 测试目标：确认查询画像接口返回的字段集合包含新增的学历、能力与自定义大项字段。 前置条件：鉴权成功，服务层返回含新增字段的画像响应。 步骤：调用 GET
     * /api/profiles/user。 断言： - HTTP 状态为 200； - 响应 JSON 中的 userId 为 2。 边界/异常：若服务层返回空画像，控制器仍返回
     * 200，交由前端处理占位文案。
     */
    @Test
    void getProfile() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/api/profiles/user").header("X-USER-TOKEN", "tkn"))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.userId").value(2L));
    }

    private List<ProfileCustomSectionDto> customSections() {
        return List.of(new ProfileCustomSectionDto("作品集", List.of(new ProfileCustomSectionItemDto("近期项目", "AI 口语教练"))));
    }
    ...
    private UserProfileResponse profileResponse() {
        return new UserProfileResponse(
                1L, 2L, "dev", "code", "learn", "master", "B2", 15, "exchange study", "沉稳而有条理", customSections());
    }
}
