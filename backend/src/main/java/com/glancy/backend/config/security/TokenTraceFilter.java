...
/** 令牌追踪过滤器：只负责记录令牌状态与请求标识，不改变既有鉴权结果。 作用：在进入安全链（Security Chain，安全链）与控制器之前，区分令牌状态并写入日志。 */
@Slf4j
public class TokenTraceFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
            @NonNull HttpServletRequest req, @NonNull HttpServletResponse resp, @NonNull FilterChain chain)
