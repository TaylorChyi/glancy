  ...
  void validate(String from) {
    if (!StringUtils.hasText(transactionalDomain)) {
      throw new IllegalStateException(
          "mail.verification.streams.transactional-domain must be configured");
    }
    if (StringUtils.hasText(marketingDomain)
        && marketingDomain.equalsIgnoreCase(transactionalDomain)) {
      throw new IllegalStateException(
