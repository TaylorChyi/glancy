...
/**
 * Centralizes creation of the Aliyun OSS client so that all services share a single, properly
 * configured instance.
 */
@Configuration
@Slf4j
  ...
  private OSS buildClient(String endpoint, OssProperties props) {
    ...
    if (props.getSecurityToken() != null && !props.getSecurityToken().isEmpty()) {
      return builder.build(
          endpoint, props.getAccessKeyId(), props.getAccessKeySecret(), props.getSecurityToken());
    }
