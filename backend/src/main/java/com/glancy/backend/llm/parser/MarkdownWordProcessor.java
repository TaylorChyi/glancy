...
final class MarkdownWordProcessor {
  ...
  private static final Pattern KEY_VALUE_PATTERN =
      Pattern.compile("^(?<key>[\\p{L}\\p{IsHan}\\s]+)[:：]\\s*(?<value>.+)$");
  ...
  MarkdownWordProcessor(
      String markdown, String fallbackTerm, MarkdownSectionResolver sectionResolver) {
  ...
  private EnumMap<MarkdownSection, SectionAppender> buildAppenders() {
    ...
    mapping.put(
        MarkdownSection.EXAMPLE, (value, split) -> example = selectFirstNonBlank(example, value));
    mapping.put(
        MarkdownSection.PHONETIC,
        (value, split) -> phonetic = selectFirstNonBlank(phonetic, value));
    return mapping;
  }
