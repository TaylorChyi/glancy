...
/**
 * 背景： - 词条类型与写作引导之前直接硬编码在 {@link WordPromptAssembler} 内，扩展其他语言困难。 目的： -
 * 将词条画像解析抽象为策略工厂，按语言路由到对应实现并通过内存模板提供可扩展的指引文案。 关键决策与取舍： - 采用策略模式将不同语言的解析逻辑解耦；指引内容统一交给模板渲染器，避免硬编码。
 * 影响范围： - Prompt 装配器与未来的多语言策略共享该组件，模板常量集中管理。 演进与TODO： - 后续可引入配置驱动的策略注册，支持运行时扩展或 A/B 实验。
 */
@Component
public class WordEntryProfileResolver {
  ...
  /**
   * 意图：根据语言与规范化后的检索词推导词条类型与写作指引。 输入：语言、归一化检索词、词典风味。 输出：包含类型标签与写作指引的 {@link EntryProfile}。 流程： 1)
   * 按语言查找策略；若未匹配使用默认策略。 2) 策略内部根据术语特征选择模板并渲染。 错误处理：当模板缺失时渲染器会抛出语义化异常以便上层捕获。 复杂度：O(n)
   * 取决于字符串遍历次数，用于检测字符类别。
   */
  ...
  private static final class DefaultEntryProfileStrategy implements LanguageEntryProfileStrategy {
    ...
    public EntryProfile resolve(String normalizedTerm, DictionaryFlavor flavor) {
      ...
      String guidance =
          renderer.render(WordPromptTemplateConstants.ENTRY_GUIDANCE_DEFAULT, Map.of());
