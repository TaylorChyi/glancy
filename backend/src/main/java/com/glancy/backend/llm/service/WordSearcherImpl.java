...
/**
 * 背景： - 词典检索从流式模式迁移到同步调用后，原始实现内联了 Prompt 拼装细节导致类体量超限。 目的： - 通过委托 {@link WordPromptAssembler}
 * 专注于消息构造，让搜索编排聚焦模型选择与结果解析。 关键决策与取舍： - 采用组合而非继承来复用 Prompt 逻辑，避免引入复杂的模板层次，同时满足 250 行以内约束。 - 将
 * tone、画像指令等策略下放到可独立测试的组件，为后续扩展多语言策略预留接口。 影响范围： - 依赖该搜索器的 controller 与服务保持不变；新增组件需要在 Spring 上下文注册。
 * 演进与TODO： - 后续可引入特性开关以支持不同 Prompt 版本动态选择。
 */
@Slf4j
@Service
