...
/**
 * 背景：
 *  - Prompt 片段此前存储在磁盘文本文件中，运行期频繁读取导致 IO 开销且不利于版本化管理。
 * 目的：
 *  - 以集中常量类的形式管理所有词典 Prompt 文本，实现零磁盘读写并便于 IDE 级检索与重构。
 * 关键决策与取舍：
 *  - 直接使用 Java Text Block 表达多行模板，避免手工拼接换行符；调用方以常量引用确保唯一来源。
 * 影响范围：
 *  - 模板渲染逻辑改为依赖内存常量，删除类路径资源文件，部署包体积略有下降。
 * 演进与TODO：
 *  - 若后续需要多版本或动态模板，可在此类增加命名空间或配置化加载能力。
 */
