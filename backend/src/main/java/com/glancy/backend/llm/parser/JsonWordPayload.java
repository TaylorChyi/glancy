...
final class JsonWordPayload {
  ...
  static ParsedWord build(
      JsonNode node, String fallbackTerm, Language fallbackLanguage, String markdown) {
  ...
  private Language resolveLanguage(Language fallbackLanguage) {
    String candidate =
        firstNonBlank(textOrNull(node.path("language")), textOrNull(node.path("\u8BED\u8A00")));
  ...
  private List<String> resolveDefinitions(
      List<String> synonyms, List<String> antonyms, List<String> related) {
