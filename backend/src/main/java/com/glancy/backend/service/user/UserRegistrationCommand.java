...
public class UserRegistrationCommand {
  ...
  public UserResponse registerWithVerification(EmailRegistrationRequest request) {
    emailVerificationService.consumeCode(
        request.email(), request.code(), EmailVerificationPurpose.REGISTER);
    return createUser(
        request.username(), request.password(), request.email(), request.avatar(), request.phone());
  }

  private UserResponse createUser(
      String username, String rawPassword, String email, String avatar, String phone) {
