...
/**
 * Translates {@link MailException} instances into structured failure records for downstream
 * policies.
 */
@Component
public class EmailDeliveryFailureClassifier {
  ...
  public EmailDeliveryFailure classify(MailException exception) {
    if (exception instanceof MailAuthenticationException authException) {
      return new EmailDeliveryFailure(
          true, EmailSuppressionStatus.MANUAL, "AUTH", safeMessage(authException));
    }
    if (exception instanceof MailSendException sendException) {
      return extractFromMailSendException(sendException);
    }
    return new EmailDeliveryFailure(
        false, EmailSuppressionStatus.SOFT_BOUNCE, null, safeMessage(exception));
  }
  ...
  private EmailDeliveryFailure extractFromMailSendException(MailSendException sendException) {
    ...
    EmailSuppressionStatus status =
        permanent ? EmailSuppressionStatus.HARD_BOUNCE : EmailSuppressionStatus.SOFT_BOUNCE;
  ...
  private boolean isPermanentFailure(String diagnostic) {
    ...
    if (lowercase.contains("mailbox full")
        || lowercase.contains("temporary")
        || lowercase.contains("try again")) {
      return false;
    }
    if (lowercase.contains("denied")
