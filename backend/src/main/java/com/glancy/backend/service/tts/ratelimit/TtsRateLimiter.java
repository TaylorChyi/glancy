...
public class TtsRateLimiter {
  ...
  public void validate(Long userId, String ip) {
    ...
    long retryIp =
        checkBucket(ipBuckets, ip, cfg.getIpPerMinute(), cfg.getBurst(), cfg.getCooldownSeconds());
    if (retryIp > 0) {
  ...
  private long checkBucket(
      Map<String, Bucket> buckets, String key, int perMinute, int burst, int cooldownSeconds) {
