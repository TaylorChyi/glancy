...
@Component
public class TemplateDrivenVerificationEmailContentResolver
    implements VerificationEmailContentResolver {
  ...
  public TemplateDrivenVerificationEmailContentResolver(
    ...
    localization
        .getMessages()
        .forEach(
            (languageTag, message) -> bodyTemplates.put(languageTag, message.getBody().trim()));
