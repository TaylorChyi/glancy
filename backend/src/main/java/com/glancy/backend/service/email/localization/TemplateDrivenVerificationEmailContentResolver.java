...
public class TemplateDrivenVerificationEmailContentResolver implements VerificationEmailContentResolver {
    ...
    public TemplateDrivenVerificationEmailContentResolver(
            EmailVerificationProperties properties, VerificationLocaleResolver localeResolver) {
        ...
        localization
                .getMessages()
                .forEach((languageTag, message) ->
                        bodyTemplates.put(languageTag, message.getBody().trim()));
    ...
    public LocalizedVerificationContent resolve(String clientIp, String code) {
        ...
        String rendered = pattern.replace("{{code}}", StringUtils.hasText(code) ? code : "")
                .trim();
        String htmlBody = "<p style=\"margin:0; font-size:16px; line-height:1.5; color:#1f2933;\">"
    ...
    private Optional<String> resolvePattern(Locale locale) {
        ...
        return bodyTemplates.entrySet().stream()
