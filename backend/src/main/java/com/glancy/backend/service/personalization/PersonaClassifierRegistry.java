...
final class PersonaClassifierRegistry {
    ...
    private static final class JobPersonaClassifier implements PersonaClassifier {
        ...
        private static final List<JobPersonaRule> JOB_RULES = List.of(
                new JobPersonaRule(List.of("学生", "student"), new PersonaProfile("专注进阶的校园学习者", "同班同学", "亲切易懂", true)),
                new JobPersonaRule(
                        List.of("老师", "教师", "teacher"), new PersonaProfile("经验分享型的教育者", "课堂上的学员", "严谨清晰", true)),
                new JobPersonaRule(
                        List.of("工程", "engineer", "开发"), new PersonaProfile("注重逻辑的工程实践者", "协作的技术同事", "结构化且高效", true)),
                new JobPersonaRule(List.of("设计", "design"), new PersonaProfile("敏锐的设计探索者", "创意共创的伙伴", "细腻而感性", true)));
    ...
    private static final class GoalPersonaClassifier implements PersonaClassifier {
        ...
        public Optional<PersonaProfile> classify(PersonaInput input) {
            ...
            return Optional.of(new PersonaProfile("以" + trimmed + "为目标的进阶者", "同样专注" + trimmed + "的伙伴", "鼓舞人心", true));
        }
    }
}
