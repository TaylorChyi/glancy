...
public class UserVerificationCommand {
  ...
  public void sendVerificationCode(EmailVerificationCodeRequest request, String clientIp) {
    ...
    try (EmailVerificationLogContext ignored =
        EmailVerificationLogContext.create(normalizedEmail, purpose)) {
  ...
  private void ensureEmailExists(String normalizedEmail) {
    ...
    userRepository
        ...
        .orElseThrow(
            () -> {
              log.warn(
                  "Login verification code requested for non-existent email {}", normalizedEmail);
  ...
  private void ensureEmailAvailable(String normalizedEmail) {
    ...
    userRepository
        ...
        .ifPresent(
            existing -> {
              log.warn(
                  "Attempt to request change-email code for already bound address {}",
                  normalizedEmail);
              throw new DuplicateResourceException("邮箱已被使用");
            });
  }
