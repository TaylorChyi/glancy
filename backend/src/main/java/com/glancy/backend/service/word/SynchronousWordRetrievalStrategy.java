...
public class SynchronousWordRetrievalStrategy implements WordRetrievalStrategy<WordResponse> {
  ...
  private final WordVersionContentStrategy defaultContentStrategy =
  ...
  private Optional<WordResponse> tryLoadFromCache(
      WordQueryContext context, SearchRecordResponse record) {
    if (context.forceNew()) {
    ...
    return cacheManager
        ...
        .map(
            word -> {
              ...
              searchRecordCoordinator.synchronizeRecordTermQuietly(
                  context.userId(), idOf(record), word.getTerm());
              return personalizationApplier.apply(
                  context.userId(), response, context.personalizationContext());
            });
  }
  ...
  private PersistenceOutcome persistResponse(
      WordQueryContext context, SearchRecordResponse record, WordResponse response) {
