...
public class WordIssueReportService {
    ...
    /**
     * 意图：将举报请求转换为持久化实体并保存，返回确认信息。 输入：用户标识 userId、举报请求体 request。 输出：持久化后的响应对象，包含数据库主键与创建时间。 流程： 1)
     * 构造实体并标准化可变字段； 2) 写入仓储； 3) 生成响应记录用于返回给调用方。 错误处理：依赖 Spring 事务回滚；若写入失败会抛出运行时异常。 复杂度：O(1) 写入。
     */
    @Transactional
