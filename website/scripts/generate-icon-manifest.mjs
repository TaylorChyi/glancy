...
const buildModuleSource = (manifest) => {
  ...
  for (const [name, variantMap] of sortedEntries) {
    ...
    for (const [variant, relativeImportPath] of sortedVariants) {
      const assetIndex = importIndex++;
      const importName = `iconAsset${assetIndex}`;
      const rawImportName = `iconAssetContent${assetIndex}`;

      importLines.push(`import ${importName} from "${relativeImportPath}";`);
      importLines.push(
        // 设计为双通道导入：URL 用于历史 <img> 兼容，原始文本用于内联渲染。
        `import ${rawImportName} from "${relativeImportPath}?raw";`,
      );
  ...
  const header = `${"//"} Auto-generated by scripts/generate-icon-manifest.mjs. Do not edit manually.`;
  const importsSection = importLines.join("\n");
  const registrySection = `const registry = Object.freeze({\n${registryLines.join(",\n")}\n});`;
