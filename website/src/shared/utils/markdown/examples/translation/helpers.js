...
export function extractValidTranslationContent(upcomingLine) {
  if (!upcomingLine) {
    return null;
  }
  const translationContent = upcomingLine.trimStart();
  return parseTranslationLine(translationContent) ? translationContent : null;
}

export function sanitizeFollowingSegments(
  trimmedExampleBody,
  translationContent,
) {
  const sanitizedSegments = sanitizeTranslationSegments(
