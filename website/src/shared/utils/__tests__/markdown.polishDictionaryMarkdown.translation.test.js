/**
 * 背景：
 *  - polishDictionaryMarkdown 在翻译行的拆分、别名兼容与括号处理上演化迅速，原始测试集中导致维护困难。
 * 目的：
 *  - 将与翻译识别相关的用例独立出来，形成聚焦的语义回归套件。
 * 关键决策与取舍：
 *  - 按标签语义归组而非按实现细节拆分，保证可读性；
 *  - 继续覆盖中英文标签与别名，确保跨语言一致。
 * 影响范围：
 *  - 改动仅限测试组织，生产代码路径保持不变；
 *  - 后续新增翻译标签或包装规则时，可在此文件集中维护。
 * 演进与TODO：
 *  - 若未来需要覆盖更多多语种标签，可考虑引入数据驱动的参数化用例以减少重复。
 */
