/**
 * 背景：
 *  - polishDictionaryMarkdown 的结构化断行逻辑涵盖列表、标题与缩进修复，原先集中在超长测试文件中。
 * 目的：
 *  - 将所有与行结构/缩进相关的回归用例收敛到独立套件，使职责清晰并符合 lint 上限。
 * 关键决策与取舍：
 *  - 继续以真实示例驱动测试，避免抽象化导致语义缺失；
 *  - 拆分后仍共享原实现，确保断行与缩进策略保持一致。
 * 影响范围：
 *  - 仅调整测试组织结构，不影响生产逻辑；
 *  - 未来如需扩展新的结构修复行为，可在此文件追加用例而无需跨文件查找。
 * 演进与TODO：
 *  - 若断行策略再次扩展到特定语言模板，可考虑按语言进一步细分测试模块。
 */
