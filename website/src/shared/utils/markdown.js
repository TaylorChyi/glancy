...
const BROKEN_HEADING_LINE_PATTERN =
  /^(#{1,6})(?:[ \t]*)\n([ \t]*)(\S[^\n]*)$/gm;
...
const HEADING_ATTACHED_LIST_PATTERN =
  /^(#{1,6}\s*)([^\n]*?)(-)(?!-)([^\n]+)$/gm;
const HEADING_INLINE_LABEL_PATTERN =
  /^(#{1,6}[^\n]*?)(\*\*([^*]+)\*\*:[^\n]*)/gm;
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (!handled) {
      ...
      if (fallbackTranslation) {
        ...
        const translationIndent = deriveExampleTranslationIndent(prefix);
        normalized.push(
          `${translationIndent}**翻译**: ${translation}`.replace(
            /[ \t]+$/u,
            "",
          ),
        );
...
export function polishDictionaryMarkdown(source) {
  ...
  const withDelimitersRestored =
    restoreMissingLabelDelimiters(withHeadingsMerged);
  ...
  const withoutDanglingSeparators =
    resolveDanglingLabelSeparators(withInlineBreaks);
