...
function KeyboardSection({ title, headingId }) {
  ...
  return (
    <PreferenceSection title={title} headingId={headingId} className={styles.section}>
            ...
            return (
              <li key={item.action} className={styles.item}>
                ...
                <button
                  ...
                  aria-label={t.settingsKeyboardEditLabel?.replace("{label}", label) ?? `Edit shortcut for ${label}`}
                  onClick={() => handleCaptureStart(item.action)}
                  onKeyDown={(event) => handleCaptureKeyDown(item.action, event)}
