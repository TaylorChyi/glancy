/**
 * 背景：
 *  - 豆包模型在词典模式下输出的 Markdown 为紧凑字段串，缺乏换行与空格，导致前端渲染层无法识别结构。
 * 目的：
 *  - 引入可扩展的格式化器工厂，对助手流式输出按策略进行预处理，恢复 Markdown 层级与可读性。
 * 关键决策与取舍：
 *  - 采用“策略模式”封装不同模型/内容类型的格式化逻辑，当前实现内置豆包词典策略与透传策略。
...
/**
 * 背景：
 *  - 豆包词典在 2024Q4 起持续扩展字段集，除早期的 senses/example 外，又新增英文释义、适用人群等模块。
 * 关键决策与取舍：
