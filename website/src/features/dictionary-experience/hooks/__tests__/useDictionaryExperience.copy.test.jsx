/**
 * 背景：
 *  - 复制反馈状态机牵涉计时器与词条缓存，单文件过长会降低维护效率。
 * 目的：
 *  - 独立验证复制成功、空 payload 与异常路径，确保状态转移符合预期。
 * 关键决策与取舍：
 *  - 通过公共 harness 统一 mock，辅以局部辅助函数压缩测试体积；
 *  - 采用顶层 describe 将复制相关用例聚合，方便后续扩展更多反馈分支。
 * 影响范围：
 *  - 仅覆盖工具栏复制按钮的核心交互，不涉及分享菜单或举报流程。
 * 演进与TODO：
 *  - 后续可加入粘贴板权限受限、并发复制等场景以完善状态机覆盖。
 */
