...
> 关联 SRS：第 4 章 4.2.2、4.5；第 5 章 FR-010/011/050/070/071；第 6 章 6.3；第 8 章 8.3–8.4；第 9 章 9.4–9.5；第 14 章 14.2；第 16 章 16.3；第 20 章 20.3 / 20.17 / 20.21（AC-UC-01）。
...
- AC-UC-01：Given 输入为 L1–L4 词或词组 When `POST /api/v1/lookup` Then 响应 200，`modules` 字段齐全、同语模式无译文，且 Header 带 `X-Cache`、`X-Quota-*`（20.18.1 / TC-API-001/002/TC-CACHE-001）。
- AC-UC-01-PERF：Given 2000 条样本 When 压测 S1 脚本 Then 首屏 P95 ≤ 2.5 s、平均 ≤ 1.2 s（TC-PERF-001）。
- AC-UC-01-Quota：Given 用户用尽配额 When 再次查询 Then 返回 429 并提示冷却时间（TC-QUOTA-001）。
- AC-UC-01-Stream：Given 正常网络 When 10 分钟监控流式指标 Then `stream_chunks_dropped/total ≤ 0.5%` 且 `aria-live` 提示一次（第 15 章 A11y-C08 / 20.13）。
