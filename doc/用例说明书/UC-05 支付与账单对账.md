...
> 关联 SRS：第 1 章 1.4.5；第 4 章 UC-08（支付）/UC-15（账单与收据）；第 5 章 FR-042/090/092；第 8 章 8.8（API-401~404）与 8.6（回执）；第 9 章 9.5（`orders`、`transactions`、`invoices`、`audit_events`）；第 11 章 11.4–11.14（商品、状态机、接口、对账）；第 20 章 20.3 / 20.12 / 20.17 / 20.21；对应 AC：AC-FR-042、AC-FR-090、AC-FR-092、AC-UC-08、AC-NFR-002。
...
- AC-FR-042（第 20 章 20.12.5，REL-B3/B4）：Given 用户访问账单页 When 调用 `GET /v1/invoices` 与 `GET /api/v1/receipts/{receiptId}` Then 返回档位、生效/到期时间、订单号、税额、优惠，收据下载链接 TTL 10 分钟且一次性，审计记录 `invoice.downloaded`（TC-BILL-001、TC-EXPORT-002）。
- AC-FR-090（第 20 章 20.12.2，REL-B1/B2）：Given 支付成功回调到达 When 订单服务更新 `orders` 状态 Then ≤5 s 同步 `transactions`、生成账单、触发 UC-04 权益生效，UI 展示“已升级”，并写入 `audit_events.billing.paid`（TC-BILL-001）。
- AC-FR-092（第 20 章 20.12.6，REL-B2/B3）：Given 渠道触发退款或拒付 When 回调生效 Then `orders`/`transactions`/`subscriptions` 幂等更新为退款状态，账单生成红冲条目，权益立即回滚并记录 `audit_events.payment.refunded`（TC-BILL-002、DR-05）。
- AC-UC-08（第 20 章 20.12.2，REL-B1/B2）：Given 订单号传递给 UC-04 When `/subscription` 查询最新档位 Then `receiptId`、`orderId` 与账单模块完全一致，对账差异率 0，若配额同步失败需回滚并告警（TC-BILL-001、订阅演练脚本）。
- AC-NFR-002（第 20 章 20.3.6，REL-B1/B5）：Given 观察 4 小时窗口 When 监控支付/账单 API 可用性与对账任务 Then 可用性 ≥99.9%，差异任务 15 分钟内处理完毕，回调原文加密存档（TC-OBS-001、Billing 对账脚本）。
