...
> 关联 SRS：第 4 章 UC-06；第 5 章 FR-020/022/080；第 8 章 8.5–8.6（API-101~103、API-201/202/403）；第 9 章 9.5（`history`、`export_jobs`、`deletion_tasks`、`audit_events`）；第 13 章 DR-04；第 20 章 20.3 / 20.11 / 20.17 / 20.21；对应 AC：AC-UC-06、AC-FR-022、AC-NFR-007、AC-FR-080、AC-NFR-012。
...
- AC-UC-06（第 20 章 20.7.3，REL-B3/B4）：Given 用户选择 CSV 并提交导出 When 调用 `POST /api/v1/exports` Then ≤5 s 收到 202 响应附 `exportId/receiptId`，后续 `GET /api/v1/exports/{id}` 在 5 s 内提供一次性下载链接（TTL=600 s）（TC-EXPORT-001、DR-04）。
- AC-FR-022（第 20 章 20.7.4，REL-B3）：Given 导出文件生成完成 When 下载链接被使用 Then 对象存储记录一次性令牌消耗，返回 UTF-8 CSV/JSON，字段与脱敏策略符合 `glancy.export.v1` 表头（TC-EXPORT-002）。
- AC-NFR-007（第 20 章 20.7.6，REL-B1/B5）：Given 用户处于无痕模式 When 发起导出 Then 系统拒绝请求并提示“无痕记录不可导出”，审计写入 `audit_events`，同时 T+7d 清理任务仍照常运行（TC-DATA-001）。
- AC-FR-080（第 20 章 20.7.7，REL-B3）：Given 导出过程中出现异常 When 任务回滚 Then 删除半成品对象、`export_jobs` 退回 `pending` 并返还配额，写入 `export.rollback` 与 `audit_events` 供审计（DR-04）。
- AC-NFR-012（第 20 章 20.5.9，REL-B2/B4）：Given 抽查导出监控 When 查看 `exports.backlog` 与成本护栏指标 Then 队列积压恢复 ≤15 分钟、成本燃率 ≤1.2x，观测面板同步展示（成本脚本、TC-OBS-001）。
