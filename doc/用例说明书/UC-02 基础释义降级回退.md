...
> 关联 SRS：第 1 章 1.4.7；第 4 章 4.2.7 / UC-14；第 5 章 FR-070；第 6 章 NFR-017；第 7 章 7.4.5；第 8 章 8.3（`degraded` 字段）；第 13 章 13.3–13.5；第 14 章 14.2；第 16 章 16.3；第 20 章 20.3 / 20.17 / 20.21（AC-UC-14）。
...
- AC-UC-14：Given Doubao 5xx ≥5% 且持续 ≥1 分钟 When 连续调用 `POST /api/v1/lookup` Then 响应 200 且 `degraded=true`、模块回退基础释义+模板例句、UI 显示退化徽标（TC-DEGRADE-001、DR-01）。
- AC-UC-14-HalfOpen：Given 熔断已开启 When 触发半开探测 Then 连续 3 次成功才闭合，否则立即回滚到 Open 并告警（DR-03 / 13.4.3）。
- AC-UC-14-Guardrail：Given 成本护栏≥95% When 再生成触发 Then 查词命中强缓存，再生成返回 403 且 `meta.kind=guardrail`（DR-06）。
