...
> 关联 SRS：第 1 章 1.4.4–1.4.5；第 4 章 UC-08/UC-09；第 5 章 FR-041/090/091/050；第 8 章 8.8（API-401~404）；第 9 章 9.4（`plans`、`subscriptions`、`quota_daily`、`entitlement_events`）；第 10 章 10.2–10.4；第 11 章 11.5–11.12；第 20 章 20.3 / 20.12 / 20.17 / 20.21；对应 AC：AC-UC-08、AC-UC-09、AC-FR-041、AC-FR-050、AC-NFR-002。
...
- AC-UC-08（第 20 章 20.12.2，REL-B1/B2）：Given 用户升级 Plus→Pro 并完成支付 When 渠道回调成功 Then ≤5 s `/api/v1/subscription` 与 `/api/v1/quotas` 反映新档位/配额，`entitlement_events` 状态为 `delivered` 且 UI 展示新权益（TC-BILL-001、TC-QUOTA-001）。
- AC-UC-09（第 20 章 20.12.3，REL-B1/B2）：Given 订阅到期或 `cancel_at_period_end=true` When 定时任务或渠道事件触发 Then 自动降级为 Free，幂等重复执行不重复扣费，历史/配额阈值同步（DR-03、TC-BILL-001）。
- AC-FR-041（第 20 章 20.12.4，REL-B1/B2）：Given 支付服务回调成功 When 更新 `subscriptions` 时配额域 5 s 内未确认 Then 触发补偿重试 ≤3 次并在 UI 提示“权益同步中”，超出后进入告警队列（订阅演练脚本、TC-QUOTA-001）。
- AC-FR-050（第 20 章 20.5.5，REL-B2）：Given 查词/再生配额变更 When 当日剩余额度需要迁移 Then 新档位额度立即生效，旧额度在账期内保留“更宽松不超限”策略 ≤5 s（TC-QUOTA-002）。
- AC-NFR-002（第 20 章 20.3.6，REL-B1/B5）：Given 观察 4 小时窗口 When 统计 `/subscription` 与 `/quotas` 可用性 Then 均 ≥99.9%，订阅变更平均延迟 ≤5 s 并在看板留痕（TC-OBS-001）。
