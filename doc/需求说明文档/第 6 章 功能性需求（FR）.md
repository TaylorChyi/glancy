...
- 内容：仅接受“单词或词组”，句子判定遵循[第 9 章「句子输入判定规范」](<./第 9 章 接口与数据契约（APISchema）.md#9100-句子输入判定规范>)，命中时前端拦截并提示改为词/词组。
- 优先级：Must
- 验收要点：
...
- 优先级：Must
- 验收要点：
  1. Free/Plus/Pro 档的例句数量、可调范围与再生成配额遵循[第 12 章](<./第 12 章 订阅、计费与账单.md>) SSoT；前端仅引用不复制数值。
...
- 验收要点：
  ...
  1. Free/Plus/Pro 档的留存天数与条数遵循[第 12 章](<./第 12 章 订阅、计费与账单.md>)定义；前端展示剩余额度并在响应体镜像 quota 对象。
...
- 优先级：Must
- 验收要点：
  1. Free/Plus/Pro 档配额遵循[第 12 章](<./第 12 章 订阅、计费与账单.md>)定义，所有写接口在响应头返回 X-RateLimit/X-Quota 系列及 X-Quota-Reset-At，并在响应体镜像 quota 对象。
...
> 详细 UC 场景见[第 5 章「业务流程与用例」](<./第 5 章 业务流程与用例.md>)；本节为 FR 到 UC 的检查点总览，供测试编排。
