...
> 说明：本章为 MVP 阶段可交付的功能清单，使用 FR-XXX 编号与 Must/Should/Could/Will not 优先级。所有功能均以第 1 章的范围、分层能力、配额与数据控制为追溯依据。
...
- 内容：仅支持 L1–L4 语言对；同语模式不显示译文（用户可临时展开）。
- 优先级：Must
- 验收要点：
  1. 非白名单语言对直接拒绝并返回指引；2) en→en、zh→zh 默认不出现译文区块；3) 切换语言对后立即生效且不需刷新。
- 追溯：语言对白名单与同语模式。

**FR-002 输入类型与句子拦截**

- 内容：仅接受“单词或词组”，输入整句时在前端拦截并提示改为词/词组。
- 优先级：Must
- 验收要点：
  1. 句号、问号、主谓结构被判定为句子时阻止提交；2) 弹出轻提示且保留原输入；3) 服务端对绕过前端的整句请求返回 4xx。
- 追溯：输入与查词规则。
...
- 内容：同一用户、同一词条、同一配置短时内重复提交仅计 1 次业务操作，返回同一结果。
- 优先级：Must
- 验收要点：
  1. 生成幂等键 hash(user_id+entry+config)；2) 3 秒内重复点击不会产生多条历史；3) 重试不计入“查词上限”。
- 追溯：术语表“幂等”“查词上限”。
...
- 内容：例句数量、再生成次数、难度与风格随订阅档位受控；同语模式不显示译文。
- 优先级：Must
- 验收要点（按档位抽测）：
  - 普通：1 条固定；再生成 1 次/日；可切换“更简单/更难”“口语/书面/学术/商务”。
  - Plus：默认 3 条，可调 1–3；再生成 10 次/日；同上。
  - Pro：默认 5 条，可调 1–5；再生成 20 次/日；支持“词条级记忆偏好”。
     切换难度/风格后仅例句区块刷新，其他区块保留。
- 追溯：分层功能说明与订阅权益。
...
- 内容：普通档支持基础开关；Plus/Pro 额外支持模块排序与详略级别选择，Pro 增加“词条级记忆偏好”。
- 优先级：Must
- 验收要点：
...
- 验收要点：
  ...
  - 普通：默认 10 天、上限 100 条；
  - Plus：默认 90 天、上限 1000 条；
  - Pro：永久保留；

  1. 开启无痕后不写入本地与后端；2) 超限队尾清理有审计事件。
...
- 内容：普通档 CSV；Plus/Pro 支持 CSV/JSON（Pro 允许条件过滤与批量导出）。
- 优先级：Must
- 验收要点：
  1. 导出任务≤5 s 返回一次性下载链接（TTL 10 分钟）与回执号；2) 字段脱敏与带表头；3) 断链需可重新申请。
- 追溯：导出能力与统一口径。
...
- 优先级：Must
- 验收要点：
  1. 画像修改后下一次生成即生效；2) 档位差异：普通≤3、Plus≤5、Pro≤8 个风格标签；3) 本地缓存与后端一致。
- 追溯：画像字段与分层限制。
...
- 优先级：Must
- 验收要点：
  - 普通：查词 30 次/日；再生成 1 次/日；
  - Plus：查词 100 次/日；再生成 10 次/日；
  - Pro：查词 500 次/日；再生成 20 次/日。
     超限返回 429 并提示剩余冷却时间。
- 追溯：订阅与权益、配额口径。
...
## 6.11 端到端性能与首屏体验（与 NFR 对齐的功能侧要求）
...
> 详细 UC 场景见第 5 章；本节为 FR 到 UC 的检查点总览，供测试编排。
