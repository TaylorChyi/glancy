...
| --- | --- | --- | --- |
| 多档日配额（`FREE_USER`/`PLUS_USER`/`PRO_USER`）、升级降级规则（10.3） | 分档次数、活动桶、幂等键、示例计算 | 仅非会员 10 次限制，会员无限制 | ISSUE#T03-QUOTA-01 |
...
- 档位划分：`FREE_USER` / `PLUS_USER` / `PRO_USER`，具体查词与再生成日配额以[第 11 章 订阅、计费与账单](<./第 11 章 订阅、计费与账单.md>)权益矩阵为唯一事实来源，此处仅引用不复刻数值。
...
| ---- | ------------------- | -------------- | ------------ | ---------------------------- |
| `FREE_USER` | 1                   | 5              | 2            | 保障偶发连点但抑制长时间冲击 |
| `PLUS_USER` | 2                   | 10             | 4            | 适配学习冲刺与批量查询       |
| `PRO_USER`  | 5                   | 20             | 6            | 面向重度用户与多端并发       |
...
- **全局级**：默认 800 RPS，突发 1600；全局保护阈值触发时按档位配额权重分配最小份额：`PRO_USER`≥60%，`PLUS_USER`≥30%，`FREE_USER`≥10%。
...
- **多级优先队列**：`PRO_USER` > `PLUS_USER` > `FREE_USER`，同级按 FIFO。
...
1. **同一词条频繁再生成**（`PLUS_USER` 用户 10 次/日已满）：
