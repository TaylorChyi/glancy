...
> 游客身份、游客会话与配额字段的能力边界参照[第 2 章 2.9](<./第 2 章 产品范围与目标.md#29-用户身份与模式定义>)，本章仅描述数据实现方式。
...
| --------------------------- | ----------------------------------------------- | ---------------------------------------- | ------------------------------------- |
| `lookups`/`results`（历史） | `FREE_USER`/`PLUS_USER`/`PRO_USER` 留存口径以[第 11 章 订阅、计费与账单](<./第 11 章 订阅、计费与账单.md>)为准（示例：`FREE_USER` 10 天/100 条、`PLUS_USER` 90 天/1000 条、`PRO_USER` 永久） | 超上限按时间滚动清理；无痕查询不落库内容 | 逻辑删除即时；物理清理延迟 7 天批处理 |
...
- **日配额**：`quota_daily` 以“用户本地自然日”为唯一口径；写入时使用 `users.time_zone`（首次登录写入真实时区，`GUEST` 会话使用浏览器时区估算）计算 `quota_date_local`，并落库到 `quota_daily.time_zone` 以供审计存证与重算。
