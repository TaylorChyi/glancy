...
- **幂等键**：`idempotency_key = hash(user_id + entry + config)`。
...
```json
{
  ...
  "isSameLang": false,
  "senses": [
    {"pos": "n.", "def": "在银行开立的账户", "examplesRef": [0]}
  ],
  "examples": [
    {"text": "I opened a new bank account.", "translation": "我新开了一个银行账户。", "difficulty": "A2", "style": ["daily"]}
  ],
  "collocations": ["open an account", "joint account"],
  "fixedPhrases": ["bank account number"],
  "synonyms": ["checking account"],
  "antonyms": [],
  "derivatives": ["banker", "banking"],
  "modules": {"definitions": true, "examples": true, "collocations": true, "synonyms": true, "derivatives": true},
  "detailLevel": "medium",
  "lengthBudget": {"zh": 150, "en": 220},
  "truncated": false
}
```
...
| --------------------------- | ----------------------------------------------- | ---------------------------------------- | ------------------------------------- |
| `lookups`/`results`（历史） | 普通 10 天/100 条；Plus 90 天/1000 条；Pro 永久 | 超上限按时间滚动清理；无痕查询不落库内容 | 逻辑删除即时；物理清理延迟 7 天批处理 |
...
- **日配额**：`quota_daily` 以用户本地时区的自然日分区；写入时使用 `users.tz` 计算 `quota_date_local`。
