...
| NFR-010 | 兼容      | 浏览器与设备兼容矩阵按[第 18 章](<./第 18 章 兼容性与客户端支持矩阵.md>)执行；降级不失能               | 兼容清单通过；关键路径功能可用                               | Must   | 19           |
...
- 零停机发布；灰度比例可配；出错自动回滚；版本策略与弃用期对齐[第 22 章](<./第 22 章 变更管理与版本策略（契约弃用期）.md>)。
...
- 浏览器/系统支持矩阵按[第 18 章](<./第 18 章 兼容性与客户端支持矩阵.md>)执行；非支持环境需给出可读提示，不导致核心流程失效。
