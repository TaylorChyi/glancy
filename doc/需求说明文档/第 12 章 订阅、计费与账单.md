...
> 本章在第 1.4.4「订阅与权益概览」、第 1.4.5「支付与账单」、第 11 章「配额、限流与成本控制」的基础上细化订阅商品、计费规则、账单票据、对账与状态机，作为订阅域的单一事实源（SSoT）。所有时间以 UTC 存储，前端按本地时区渲染；下载链接 TTL、删除语义等遵循文档统一约定。

------

## 12.1 目标与范围

- **目标**：以最低耦合实现三档订阅（普通/Plus/Pro）的售卖、续费、升级降级、账单与收据、退款与争议、对账与审计，确保权限与配额在 5 s 内一致。
- **范围**：Web/H5（外部支付）、iOS 内购、Android 应用商店计费；个人账户模型；不含组织/家庭共享与礼品卡。
...
| 名称         | 定义                             | 关键字段                                                     |
| ------------ | -------------------------------- | ------------------------------------------------------------ |
| Plan         | 订阅档位（普通/Plus/Pro）        | plan_id、name、entitlements                                  |
...
> 计费与配额两个域通过 entitlement 同步，见 12.12 与第 11 章。
...
- **价格来源**：
  - iOS/Android：以商店侧价格为准，服务端维护映射 `price_id ↔ store_product_id`。
  - Web/H5：以后台配置价为准，可灰度。
- **展示**：价格与权益并排展示；权益取自第 1.4.4 表格并作为渲染数据源。
...
- **iOS/Android**：商店托管续费；我们仅据回调/票据校验入账与改状态。
...
- **通道**：
  - iOS：内购；票据验证与服务器通知驱动状态。
  - Android：应用商店计费；订阅状态由服务器通知驱动。
  - Web/H5：外部支付（例如扫码支付）；一次性扣款，无循环代扣。
...
- **回调**：落库后以幂等更新订单、交易、订阅三表；多次回调仅一次生效。
- **轮询**：对账任务每 15 分钟增量，T+1 全量校验；差异以“本地为准/渠道为准”可配置。
...
- **映射**：Plan → 限额（每日查词、再生成、历史留存上限、导出能力、模块自定义能力）来自第 1.4.4 表。
...
- **业务事件**：`order.created/paid/failed、subscription.activated/renewed/canceled/expired/upgraded/downgraded、billing.invoice.issued、refund.succeeded、chargeback.accepted`
...
- **UC-12-01 开通订阅（Web/H5）**：下单→支付→回调→ACTIVE→权益到位。
- **UC-12-02 自动续费成功（iOS/Android）**：商店续费→服务器通知→续期改写→权益延长。
- **UC-12-03 升级订阅**：即时补差→权限即时提升→配额上限变更。
- **UC-12-04 降级到期生效**：设置到期不续/降级→当前周期不变→到期 EXPIRED/新 Plan 生效。
- **UC-12-05 退款（全额）**：平台退款→状态 REFUNDED→权益移除→账单红冲。
- **UC-12-06 拒付**：渠道争议成立→REVOKED→立即移除权益并告警。
- **UC-12-07 收据下载**：账单详情生成一次性链接→10 分钟有效→可重复申请。

------

**与前文衔接说明**：

- 权益与限额的具体数值、模块自定义能力、历史留存与导出能力，均以第 1.4.4 表为准；配额重置口径与计量统一规则见附表。
- 幂等键、回放样本、契约版本、下载链接 TTL、审计与归档口径与附录保持一致。
