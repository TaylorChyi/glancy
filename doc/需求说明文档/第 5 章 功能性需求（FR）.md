...
> 游客模式的身份与能力边界引用[第 2 章 2.9](<./第 2 章 产品范围与目标.md#29-用户身份与模式定义>)，本章仅复用枚举名，不再另行约束。
...
- **约束**：`FREE_USER` 仅支持开关；`PLUS_USER`/`PRO_USER` 支持排序+详略，`PRO_USER` 额外支持词条级偏好；详略级别映射统一字符上限；设置需跨端同步。
...
- **约束**：`FREE_USER` 提供 CSV；`PLUS_USER`/`PRO_USER` 额外支持 JSON（`PRO_USER` 允许条件过滤/批量导出）；导出产物脱敏、UTF-8 带表头；对象存储链接 TTL 10 分钟且一次性。
...
- **约束**：采集字段（学习目标、专业背景、年龄层次、当前水平、语言对、风格标签）遵循[第 3 章 3.5](<./第 3 章 用户画像与角色.md#35-画像字段、采集与使用规则>)；风格标签数量随档位（`FREE_USER`≤3、`PLUS_USER`≤5、`PRO_USER`≤8）；支持多选排序并可撤回同意。
...
- **触发**：订阅到期、撤销或退款事件。
- **约束**：自动降级至 `FREE_USER` 并同步能力边界；超过新档位的自定义项需回退到默认；历史记录受新留存上限限制。
...
- **触发**：支付平台触发退款、拒付或对账异常。
- **约束**：状态与权限保持幂等同步；退款完成后档位恢复至到期前状态或 `FREE_USER`；账单需保留痕迹。
