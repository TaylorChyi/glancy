...
## 11.0 对齐结果（T03，2025-02）

> 当前代码尚未实现订阅/计费域，会员状态完全由 `users` 表中的 `membershipType`+`membershipExpiresAt` 控制。本节列出现状与缺口，11.1 起保留原 SRS 目标描述。

### 11.0.1 当前实现表

| 能力 | 实现方式 | 细节 | 溯源 |
| --- | --- | --- | --- |
| 会员字段 | `users.member`（布尔）、`membershipType`（`NONE/PLUS/PRO/PREMIUM`）、`membershipExpiresAt` | `User.hasActiveMembershipAt` 根据类型与过期时间判断；无 Plan/Price 表 | [User](../../backend/src/main/java/com/glancy/backend/entity/User.java) |
| 会员命令 | `UserMembershipCommand` 包装 `MembershipLifecycleService` 的 `activateMembership/removeMembership` | 仅被内部调用（例如兑换码），没有公开 API | [UserMembershipCommand](../../backend/src/main/java/com/glancy/backend/service/user/UserMembershipCommand.java)，[MembershipLifecycleService](../../backend/src/main/java/com/glancy/backend/service/membership/MembershipLifecycleService.java) |
| 兑换码 | `RedemptionCodeService` 创建/兑换码，兑换后通过 `membershipLifecycleService.extendMembership` 延期 | 是目前唯一能改变会员状态的流程，无订单/支付对接 | [RedemptionCodeService](../../backend/src/main/java/com/glancy/backend/service/redemption/RedemptionCodeService.java) |
| 客户端展示 | `LoginResponse`/`UserResponse` 返回 `membershipType`、`membershipExpiresAt`；前端仅读取字段显示徽章 | 无 `/subscription` API、无账单页 | [LoginResponse](../../backend/src/main/java/com/glancy/backend/dto/LoginResponse.java)，[website/src/app/pages](../../website/src) |
| 计费/订单/账单/退款 | 未实现 | — | — |
| 事件 / 对账 | 无订阅事件、无账单导出、无发票/收据 | — | — |

### 11.0.2 目标契约表（原 SRS 摘要）

| 模块 | SRS 约定 | 当前状态 | Issue |
| --- | --- | --- | --- |
| 商品矩阵（11.4） | `plans` + `prices`（monthly/yearly、currency），权益矩阵为 SSoT | 无表、无配置 | ISSUE#T03-BILL-01 |
| 生命周期 / 状态机（11.5） | `TRIALING→ACTIVE→GRACE→…`，自动降级与 5 s 权益同步 | 仅布尔字段，缺少状态机与事件 | ISSUE#T03-BILL-02 |
| 购买 / 支付流程（11.6） | 订单、交易、回调、轮询、对账 | 完全缺失 | ISSUE#T03-BILL-03 |
| 升降级、取消、退款（11.6.2–11.7） | `POST /v1/subscriptions/upgrade` 等 API、部分退款策略 | 无 API / 表支撑 | ISSUE#T03-BILL-04 |
| 账单/票据（11.8） | `invoices`、`receipts`、下载链接 TTL | 无 | ISSUE#T03-BILL-05 |
| 对账与事件（11.9–11.10） | `order.*`、`subscription.*`、`billing.*` 事件，SLA ≤5 s | 无事件/监控 | ISSUE#T03-BILL-06 |

### 11.0.3 Issues（订阅 / 计费）

- **ISSUE#T03-BILL-01**：需建立 `plans/prices/subscriptions/orders/transactions` 等表，明确档位权益与价格，供 9 章引用。
- **ISSUE#T03-BILL-02**：缺少订阅状态机与自动降级逻辑，无法满足 11.5 以及 10.3 “升级/降级即时生效”的要求。
- **ISSUE#T03-BILL-03**：购买/续费/支付/对账流程未实现，`RedemptionCode` 只是临时手段。
- **ISSUE#T03-BILL-04**：无升降级、取消、退款 API，无法处理用户主动操作或商店回调。
- **ISSUE#T03-BILL-05**：账单、收据、发票导出缺失，SLA/TTL 无法验证。
- **ISSUE#T03-BILL-06**：缺少订阅事件（`subscription.updated` 等）与权益同步 SLO，10 章/14 章无法联动。

------

## 11.1 目标与范围

- **目标**：以最低耦合实现三档订阅（`FREE_USER`/`PLUS_USER`/`PRO_USER`）的售卖、续费、升级降级、账单与收据、退款与争议、对账与审计，确保权限与配额在 5 s 内一致。
...
| 名称         | 定义                             | 关键字段                                                     |
| ------------ | -------------------------------- | ------------------------------------------------------------ |
| Plan         | 订阅档位（`FREE_USER`/`PLUS_USER`/`PRO_USER`）        | plan_id、name、entitlements                                  |
...
- **幂等键**：定义、哈希参数与冲突处理参见[附录 A 术语与缩略语](<./Appendix/附录 A 术语与缩略语.md#idempotency-key>)；本节遵循同一口径执行。
