...
> 目的：给出覆盖性能、可用性、可靠性、安全、隐私、可维护性、可观测性、兼容性、国际化与可访问性、成本与配额等的跨域约束，作为第 8–19、21–23 章的上位约束与验收门槛。优先级采用 Must/Should/Could/Will not。
...
- 关联章节：性能与容量（第 17 章）、可用性与容灾（第 14 章）、可观测性（第 15 章）、国际化与可访问性（第 16 章）、配额与成本（第 11 章）、安全与合规（第 13 章）。
...
| ------- | --------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------ | ------------ |
| NFR-001 | 性能      | 首屏查词端到端延迟：P95 ≤ 2.5 s；平均 ≤ 1.2 s                | 线上 APM 5 分钟滚动窗口，按端到端口径；灰度与压测回归必须过线 | Must   | 1.6、17      |
| NFR-002 | 可用性    | 关键接口月度可用性 ≥ 99.9%（lookup、regenerate、history、export） | 合法请求 2xx/3xx 占比；维护窗口不计入                        | Must   | 14、15       |
| NFR-003 | 退化      | 依赖异常时 100% 自动降级为“基础释义 + 模板例句”，并在 UI 明示 | 演练与演习记录；依赖 5xx≥5% 且持续 1 min 触发熔断            | Must   | 1.4.7、14    |
| NFR-004 | 可扩展    | 单一实例水平扩展线性（QPS 翻倍，延迟 P95 增幅 ≤ 20%）        | 压测报告与容量曲线                                           | Should | 17           |
| NFR-005 | 一致性    | 幂等：查词与再生成在重试/回放场景仅一次生效                  | 幂等键=hash(user_id+entry+config) 命中率 100%                | Must   | 附录术语     |
| NFR-006 | 安全      | 传输与存储加密；最小权限 RBAC；敏感字段脱敏                  | TLS1.2+、AES-256 at-rest、密钥轮转 ≥ 90d                     | Must   | 13           |
| NFR-007 | 隐私      | 无痕查询不落库原文与结果；用户可导出/删除                    | 黑盒审计与数据抽查为 0；导出回执 ≤ 5 s                       | Must   | 1.4、1.6、13 |
| NFR-008 | 观测      | 全链路 trace_id 贯穿前后端与第三方                           | 99% 请求具备 trace_id；日志与指标三分法齐全                  | Must   | 15、术语     |
| NFR-009 | 维护      | 零停机发布；支持灰度/回滚/契约回放 ≥ 500 样本                | 发布手册与 CI 记录；契约回放通过率 100%                      | Must   | 22、23、术语 |
| NFR-010 | 兼容      | 浏览器与设备兼容矩阵按第 19 章执行；降级不失能               | 兼容清单通过；关键路径功能可用                               | Must   | 19           |
| NFR-011 | i18n/a11y | UI 语言 zh/en；WCAG 2.1 AA 基线                              | 键盘可达、语义标签、对比度达标                               | Must   | 16           |
| NFR-012 | 成本      | 单次查词平均 tokens_out 控制在预算内；成本报表按日对账       | 成本/请求统计；对账差异 ≤ 1%                                 | Should | 11、术语     |
| NFR-013 | 合规      | 日志与审计可追溯、不可篡改；子处理者清单留档                 | 审计抽样可回放；DPA/SCC 留存                                 | Must   | 13、附录     |

------

## 7.3 性能与容量

> 与 1.6 的目标一致，并细化配额内的端到端性能预算与接口级 SLO。

### 7.3.1 端到端预算（单次查词 Lookup）

| 分段                         | 预算（P95）               | 验收口径         |
| ---------------------------- | ------------------------- | ---------------- |
| 前端交互与输入校验           | ≤ 100 ms                  | Web Vital + 自测 |
| 边缘/网关（TLS、鉴权、路由） | ≤ 150 ms                  | APM 分段         |
| 后端业务编排与缓存命中路径   | ≤ 200 ms                  | APM 分段         |
| Doubao API 调用（含重试）    | ≤ 900 ms；超时 3.0 s      | 外呼指标         |
| 结构化解析与裁剪             | ≤ 100 ms                  | 解析器计时       |
| 首屏渲染（含模块骨架）       | ≤ 200 ms                  | RUM              |
| 合计（首屏）                 | P95 ≤ 2.5 s；平均 ≤ 1.2 s | 端到端           |
...
| POST /export     | ≥ 99.0%       | 生成 ≤ 5 s | 返回回执与一次性链接，TTL 10 min |
...
- 用户可导出/删除全部历史；导出支持 CSV/JSON，≤5 s 返回回执与一次性链接（TTL 10 min）。
...
- 零停机发布；灰度比例可配；出错自动回滚；版本策略与弃用期对齐第 23 章。
...
- 浏览器/系统支持矩阵按第 19 章执行；非支持环境需给出可读提示，不导致核心流程失效。
...
- 达到 WCAG 2.1 AA：键盘可达、焦点可见、语义标签、表格关联、ARIA 合理；颜色对比度达标；图标需替代文本。第 16 章细化验收项。
...
- 查词与再生成分别计数；超限返回 429 并返回剩余冷却时间；限流口径先用户级、再租户级、最后全局。
...
## 7.12 业务连续性（RPO/RTO 概览）

**NFR-040（Must）**：RTO/RPO 基线

- 历史记录与配置：RTO ≤ 30 min，RPO ≤ 5 min；导出任务：RTO ≤ 15 min。具体演练与备份在第 14 章细化。
