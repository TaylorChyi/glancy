...
- 关联章节：性能与容量（[第 17 章](<./第 17 章 端到端性能与容量规划.md>)）、可用性与容灾（[第 14 章](<./第 14 章 可用性、容灾与备份.md>)）、可观测性（[第 15 章](<./第 15 章 可观测性与运维（日志、监控、告警、SLO）.md>)）、国际化与可访问性（[第 16 章](<./第 16 章 国际化与可访问性.md>)）、配额与成本（[第 11 章](<./第 11 章 配额、限流与成本控制.md>)）、安全与合规（[第 13 章](<./第 13 章 安全、隐私与合规.md>)）。
...
| ------- | --------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------ | ------------ |
| NFR-001 | 性能      | 首个内容模块渲染完成：P95 ≤ 2.5 s；平均 ≤ 1.2 s                | 线上 APM 5 分钟滚动窗口，按端到端口径；灰度与压测回归必须过线 | Must   | 1.6、17      |
...
| NFR-010 | 兼容      | 浏览器与设备兼容矩阵按[第 19 章](<./第 19 章 兼容性与客户端支持矩阵.md>)执行；降级不失能               | 兼容清单通过；关键路径功能可用                               | Must   | 19           |
...
- 零停机发布；灰度比例可配；出错自动回滚；版本策略与弃用期对齐[第 23 章](<./第 23 章 变更管理与版本策略（契约弃用期）.md>)。
...
- 浏览器/系统支持矩阵按[第 19 章](<./第 19 章 兼容性与客户端支持矩阵.md>)执行；非支持环境需给出可读提示，不导致核心流程失效。
...
- 达到 WCAG 2.1 AA：键盘可达、焦点可见、语义标签、表格关联、ARIA 合理；颜色对比度达标；图标需替代文本。[第 16 章](<./第 16 章 国际化与可访问性.md>)细化验收项。
...
- 历史记录与配置：RTO ≤ 30 min，RPO ≤ 5 min；导出任务：RTO ≤ 15 min。具体演练与备份在[第 14 章](<./第 14 章 可用性、容灾与备份.md>)细化。
